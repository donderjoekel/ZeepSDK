...
{
    private static readonly MethodInfo HashMethod = ResolveHashMethod();

    internal static byte[] Hash(byte[] bytes)
    {
        return (byte[])HashMethod.Invoke(null, new object[] { bytes });
    }

    private static MethodInfo ResolveHashMethod()
    {
